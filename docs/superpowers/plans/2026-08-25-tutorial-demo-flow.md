...
## 미검증 항목 (본편 빌드 시 확인 필요)

체험 플로우 작업 중 본편 코드를 함께 수정했으나, 본편은 끝까지 플레이해야 도달하는 구간이라
이번 세션에서 검증하지 못한 항목이다. **본편 빌드를 뽑을 때 반드시 확인한다.**

| # | 항목 | 확인 방법 | 관련 변경 |
|---|---|---|---|
| 1 | **본편 크레딧 → 타이틀 복귀** | 본편을 클리어해 에필로그 → 크레딧까지 재생하고, 크레딧이 끝나면 타이틀로 정상 복귀하는지 | `UI_Credits`에 `PlayMode` 분기와 `_isFinished` 가드를 추가했다. 본편은 `else` 분기로 기존과 동일한 `EnterTitle()`을 타야 한다 |
| 2 | **본편 크레딧 반복 재생** | 크레딧 → 타이틀 → 다시 본편 클리어 시 크레딧이 처음부터 스크롤되는지 | `UI_Credits.OnEnable()`에서 시작 위치를 복원하도록 했다. 본편 경로는 `Scene_UI`를 언로드하지 않으므로 이 복원에만 의존한다 |
| 3 | **본편 에필로그 진입** | 9번째 맵 트리거 → `EnterEpilogue()` → 크레딧 순서가 유지되는지 | `EnterEpilogue()` 자체는 수정하지 않았으나 `AmonEndPhase()`에 데모 분기가 추가되었다. 본편은 분기를 타지 않고 기존 리스폰 로직으로 가야 한다 |
| 4 | **본편 몬스터 스탯** | 데모 플레이 후 타이틀 복귀 → 본편 진입 시 몬스터 체력/공격력이 원본인지 | 세션 중 확인 완료(정상). 빌드에서 한 번 더 본다 |

> 1·2번은 `UI_Credits`가 본편과 데모의 **공유 지점**이라 회귀 위험이 있는 곳이다.
> 3번은 `PlayMode`가 `Normal`이면 분기하지 않으므로 구조적으로 안전하지만, 실측이 없다.
