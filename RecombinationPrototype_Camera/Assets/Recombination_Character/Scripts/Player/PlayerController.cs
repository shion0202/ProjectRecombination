...
public class PlayerController : MonoBehaviour, PlayerActions.IPlayerActionMapActions
{
    ...
    {
        if (context.performed && _canDash && !_followCamera.IsZoomed)
        {
    ...
    {
        // if (!_followCamera.IsZoomed && _moveDirection.magnitude <= 0.1f) return;
    ...
    {
        _dashStartPos = _followCamera.CameraTarget.position;
        _isDashing = true;
        _canDash = false;
        _isShooting = false;
