...
public class BallLegs : MonoBehaviour, IPartAbility
{
    ...
    private void Awake()
    {
        defaultMyPos = new Vector3(0.0f, -0.9f, -0.05f);
    }
    ...
    private void Update()
    {
        ...
        if (isIncrease)
        {
            ValueA += Time.deltaTime * 80.0f;
            float yValue = Time.deltaTime * 0.6f;
            if (ValueA >= 100.0f)
            {
                ValueA = 100.0f;
            }
            if (modelPos.localPosition.y - yValue <= -0.7f)
            {
                yValue = 0.0f;
            }
        ...
        if (owner != null && owner.FallVelocity.y <= 0.0f)
        {
            ...
            float yValue = Time.deltaTime * 1.0f;
            if (ValueB <= 0.0f)
            {
                ValueB = 0.0f;
            }
