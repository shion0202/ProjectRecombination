...
public class AmonPhase2 : MonoBehaviour
{
    ...
    private IEnumerator SoulAbsorb(Blackboard data)
    {
        // 1. 캐스팅 시작
        ...
        barrierInstance = Utils.Instantiate(barrierPrefab, transform);
        barrierInstance.transform.localPosition = barrierOffset;
        barrierInstance.transform.localRotation = Quaternion.identity;
        ...
        float elapsed = 0f;
        float castTime = 15.0f;
        while (elapsed < castTime)
        {
            Debug.Log("Hp: " + data.CurrentHealth + "/" + data.MaxHealth);

            Vector3 direction = _target.transform.position - data.Agent.transform.position;
            direction.y = 0;
        ...
        float absorbAmount = _target.Stats.MaxHealth * 0.2f;
        _target.ApplyDamage(absorbAmount, LayerMask.GetMask("Player"), 1.0f, 1.0f);
    ...
    private void Update()
    {
        ...
        {
            StartCoroutine(SoulAbsorb(blackboard));
        }
